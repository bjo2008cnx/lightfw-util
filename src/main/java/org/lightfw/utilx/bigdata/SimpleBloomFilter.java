...
/**
 * 传统的Bloom filter 不支持从集合中删除成员,Counting Bloom filter由于采用了计数，因此支持remove操作。
 */
public class SimpleBloomFilter {
    // DEFAULT_SIZE为2的25次方
    private static final int DEFAULT_SIZE = 2 << 24;  //对应的容量为33554432
    ...
    public void add(String value) {
        for (SimpleHash hash : hashes) {
            int hashedValue = hash.hash(value);
            bits.set(hashedValue, true);
        }
    }
